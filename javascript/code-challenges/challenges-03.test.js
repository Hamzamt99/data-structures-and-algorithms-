...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  let tempArray = arr.filter(item => item.baseStat > minBaseStat ? true : false);
  return tempArray;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  let tempArray = arr.filter(item => item.baseStat > minBaseStat ? true : false);
  let tempArrayName = [];
  tempArray.forEach(element => tempArrayName.push(element.stat.name));
  return tempArrayName;
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  let tempArray = arr.filter(item => !item.children ? true : false);
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let filterArray = arr.filter(item => typeof item === 'string' ? false : true);
  let tempArray = filterArray.map(item => item % 2 === 0 ? 'even' : 'odd');
  return tempArray;
};
